...
const Schedule = () => {
  ...
  const saveCell = async (scheduleName: string, day: number, time: string) => {
    ...
    if (!tempValue.trim()) {
      ...
      const { data } = await supabase.from('schedule').insert({
        user_id: user!.id, day_of_week: day, time_slot: time, content: tempValue.trim(), schedule_name: scheduleName,
      }).select().single();
      if (data) {
        setGrids(prev => ({
          ...prev,
  ...
  return (
    <div className="min-h-screen bg-background max-w-4xl mx-auto">
      <header className="gradient-hero px-5 pt-8 pb-6 rounded-b-3xl flex items-center justify-between">
        ...
        {showAddForm ? (
          <div className="flex gap-2">
            <Input
              ...
              placeholder="Nombre"
              className="h-8 text-sm w-32 bg-primary-foreground/20 border-0 text-primary-foreground placeholder:text-primary-foreground/50"
              onKeyDown={e => e.key === 'Enter' && addSchedule()}
              autoFocus
            />
            <Button size="sm" variant="ghost" className="h-8 text-primary-foreground" onClick={addSchedule}>OK</Button>
            <Button size="sm" variant="ghost" className="h-8 text-primary-foreground" onClick={() => setShowAddForm(false)}>✕</Button>
          </div>
        ) : (
          <Button size="sm" variant="ghost" className="h-8 gap-1 text-primary-foreground hover:bg-primary-foreground/20" onClick={() => setShowAddForm(true)}>
            ...
            return (
              <div key={scheduleName}>
                <div className="flex items-center justify-between px-2 mb-2">
                  <h2 className="font-bold text-foreground text-sm">{scheduleName}</h2>
